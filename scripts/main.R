...
coord_sj <- getbb("San Juan, Argentina")
...
public_transport <- getbb("San Juan in Argentina") %>%
  ...
  add_osm_feature(
    ...
    value = c("stop_position", "platform", "station", "stop_area")
  ) %>%
  osmdata_sf()
...
shops <- getbb("San Juan in Argentina") %>%
  ...
  add_osm_feature(
    key = "shop",
    value = c("supermarket", "kiosk", "general")
  ) %>%
  osmdata_sf()
...
ggplot() +
  ...
  coord_sf(xlim = c(-68.575, -68.48), ylim = c(-31.59, -31.5), expand = FALSE)
...
ggplot() +
  ...
  coord_sf(
    xlim = c(coord_sj[1, 1], coord_sj[1, 2]),
...
ggplot() +
  ...
  geom_sf(
    data = public_transport$osm_lines,
    inherit.aes = FALSE,
    color = "red",
    size = .5,
    alpha = .8
...
bsas_map <- ggplot() +
  ...
  geom_sf(
    data = electrical_bsas$osm_lines,
    inherit.aes = FALSE,
    color = "yellow",
    size = .4,
    alpha = .6
  ...
  geom_sf(
    data = public_transport_bsas$osm_lines,
    inherit.aes = FALSE,
    color = "white",
    size = 1,
    alpha = 1
  ...
  coord_sf(
    xlim = c(coord_bsas[1, 1], coord_bsas[1, 2]),
